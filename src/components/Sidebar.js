...
const Sidebar = ({ user, closeToggle }) => {
  ...
  return (
    <div className='flex flex-col justify-between bg-white h-full overflow-y-scroll min-w-210 hide-scrollbar'>
      <div className='flex flex-col'>
        ...
        <div className='flex flex-col gap-5'>
          ...
          {categories.slice(0, categories.length - 1).map((category) => {
            return (
              <NavLink to={`/category/${category.name}`} className={({ isActive }) => (isActive ? isActiveStyle : isNotActiveStyle)} key={category.name} onClick={handleCloseSideBar}>
      ...
      {user && (
        <Link to={`user-profile/${user._id}`} className='my-5  mb-3 gap-2 p-2 rounded-lg shadow-lg  bg-white flex justify-start items-center mx-3'>
